...
export type CreditIndustryTheme = {
  key: string;
  title: string;
  description: string;
...
export const CREDIT_INDUSTRY_THEMES: CreditIndustryTheme[] = [
  {
    key: "auto",
    title: "大汽车",
    description:
...
export function getCreditThemeForStandard(
  standard: CreditStandard,
): CreditIndustryTheme | undefined {
  const sectorCatI = normalizeSectorCatI(standard.sector_cat_i);
  if (!sectorCatI) return undefined;

  return CREDIT_INDUSTRY_THEMES.find((theme) =>
    theme.sectorCatIValues.includes(sectorCatI),
  );
}

export function getCreditStandardsByTheme(
  standards: CreditStandard[],
  theme: CreditIndustryTheme,
): CreditStandard[] {
...
export type CreditThemeStats = {
  standardCount: number;
  customerTags: string[];
};

export function getCreditThemeStats(
  standards: CreditStandard[],
  theme: CreditIndustryTheme,
): CreditThemeStats {
  const themeStandards = getCreditStandardsByTheme(standards, theme);
  const customerTags = [
    ...new Set(
      themeStandards
        .map((standard) => standard.sector_cat_iii?.trim())
        .filter((value): value is string => Boolean(value)),
    ),
  ];

  return {
    standardCount: themeStandards.length,
    customerTags,
  };
}
